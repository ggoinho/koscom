...
import com.sendbird.syncmanager.utils.KosSharedPreferences
...
//        presenter.initViewModel(viewModelFactory)
    ...
    private fun initListener(){
        ...
        tvComplete.setOnClickListener {
            ...
            if(tvMenuLocationLeft.isSelected){
                KosSharedPreferences(this).setInt(Keys.KEY_MENU_LOCAION, MenuLocationType.MENU_LEFT.type)
            }else{
                KosSharedPreferences(this).setInt(Keys.KEY_MENU_LOCAION, MenuLocationType.MENU_RIGHT.type)
            }

            if(ivHomeBtnExposure.isSelected){
                KosSharedPreferences(this).setBoolean(Keys.KEY_HOMEBTN_EXPOSURE, true)
            }else{
                KosSharedPreferences(this).setBoolean(Keys.KEY_HOMEBTN_EXPOSURE, false)
            }

            if(tvOrderStatusSelect.isSelected){
                KosSharedPreferences(this).setInt(Keys.KEY_HOME_CONTENTS, HomeContentsType.ORDER_SATUS.type)
            }else{
                KosSharedPreferences(this).setInt(Keys.KEY_HOME_CONTENTS, HomeContentsType.BUSINESS_INFO.type)
            }
    ...
    private fun initRightList(){
        ...
        val recentlyDrawerMenu = KosSharedPreferences(this).getString(Keys.KEY_QUICK_MENULIST)
        val recentlyMenu = Gson().fromJson(recentlyDrawerMenu, object : TypeToken<ArrayList<DrawerMenuData>>() {}.type) as ArrayList<DrawerMenuData>

        for(item in recentlyMenu){
            adapterRight.addItem(item)
        }
        adapterRight.notifyDataSetChanged()
    }
    ...
    private fun syncLeftRight(){
        for((i, item) in adapterRight.getList().withIndex()){
            for((j, leftData) in adapterLeft.getList().withIndex()){
                if(item.title == leftData.title){
                    adapterLeft.getList()[j].isSelectedMenu = true
                    adapterLeft.getList()[j].childPosition = i
                    break
                }
            }
        }
